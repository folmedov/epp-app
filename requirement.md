...
- [ ] **3.16 Tests & Data Integrity**: Unit/integration tests for mapping, upsert, and reconciliation logic. Also includes a **sample-based integrity check**: periodically re-fetch a small random subset of `job_offers` from the source APIs and compare key fields (state, salary, dates) to detect data drift between DB and upstream. (details: docs/sprints/sprint_3_14_tests_migration.md)
