...
- [x] **6.8 Hook into `ingest_all.py`**: Call `notify_new_offers.py` after `close_stale_offers.py` (non-fatal, forwards `--dry-run`).
- [x] **6.9 Queue cleanup script** (`scripts/cleanup_notification_queue.py`): Purge old `notification_queue` rows to keep the table lean. Policy: delete `sent` rows older than 30 days and `pending` rows older than 7 days (likely orphaned by crashes). Standalone cron, non-fatal.
- [x] **6.10 Fix - Notify only offers `postulacion` state**: Only offers in `postulacion` must be sent in notifications. This is a business rule change that applies to both the immediate notification and the weekly digest.
- [x] **6.11 Env vars & Dokploy crons**: `SMTP_*` vars + `APP_BASE_URL`. Three new cron services: `eepp-worker-weekly-digest` (Mondays 09:00), `eepp-worker-queue-cleanup` (daily 02:00).
- [x] **6.12 Notification error handling & diagnostics**: Fix bug where `notification_queue` rows remain stuck in `pending` on SMTP failure and `notified_at` is stamped on all offers regardless of send success. Create a diagnostic script to test SMTP connectivity independently. (details: docs/sprints/sprint_6_12_notification_error_handling.md)
...
- [x] **10.2 Pill toggle & contrast**: Redesign search-subscription term pills so the entire pill (label + count badge) is clickable to toggle active/inactive, replacing the separate icon-based toggle button. Improve visual contrast between active and inactive states: active pills use solid indigo border (2px) with violet count badge; inactive pills use dashed gray border with reduced opacity and lighter gray badge. (details: docs/sprints/sprint_10_2_pill_toggle_contrast.md)
