...
|is_active|Boolean|Visibility flag. `True` by default. Set to `False` automatically by `scripts/close_stale_offers.py` when `close_date < today` and the offer no longer appears in any active TEEE/EEPP feed. **Never modifies `state`** — the portal-reported state is preserved. The UI default query applies two conditions together: `is_active = True` AND (`close_date IS NULL OR close_date >= CURRENT_DATE`). A "Vencidas" toggle passes `include_inactive=true` to the API, removing both conditions. Added in Sprint 6.1.|
